...
in question. */
    SELECT 
        ...
        CASE WHEN monthlymaintenance<100 THEN 'cheap'
            ELSE 'expensive' END AS costlabel
    FROM Facilities; 
...
the member name. */
    SELECT DISTINCT tm.memid, tm.name, tm.surname, tm.firstname
    FROM (
        SELECT 
            b.memid, b.facid, f.name, m.surname, m.firstname
        FROM Bookings AS b
        LEFT JOIN Facilities AS f
        ON b.facid = f.facid
        LEFT JOIN Members AS m
        ON b.memid = m.memid
        WHERE f.name LIKE 'Tennis%') AS tm
    ORDER BY tm.surname
...
/* Q12: Find the facilities with their usage by member, but not guests */
        ...
        SELECT 
            ...
            --(g.guestusage+m.memberusage) OVER() AS total_usage 
        FROM Bookings AS b
            LEFT JOIN Facilities AS f
                ON b.facid = f.facid
